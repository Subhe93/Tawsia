 ...
 * 
 * الاستخدام:
 * npx ts-node scripts/migrate-existing-sitemap.ts
 */
...
async function parseSitemapXml(filePath: string): Promise<string[]> {
...
async function resolveEntityIds(entityIds: any, entryType: string) {
  const result: any = {
    companyId: null,
    countryId: null,
    cityId: null,
  ...
  for (const url of urls) {
    try {
      ...
      await prisma.sitemapEntry.create({
        data: {
          entryType: analysis.entryType as any,
          slug: analysis.slug,
          url: url,
      ...
      console.error(`❌ فشل في معالجة: ${url}`, error);
      stats.failed++;
    }
  }
