...
export const getHomePageData = cache(
  async (countryCode?: string): Promise<HomePageData> => {
    try {
      ...
      const [
        ...
        // الشركات المميزة (filtered by country if provided)
        prisma.company.findMany({
          where: {
            isActive: true,
            isFeatured: true,
            ...(countryCode && { city: { country: { code: countryCode } } }),
          },
          select: {
            id: true,
            slug: true,
        ...
        // الفئات مع عدد الشركات النشطة (filtered by country if provided)
        ...
        // أحدث المراجعات المعتمدة (filtered by country if provided)
          ...
          select: {
            ...
            company: {
              select: {
                ...
                slug: true,
                city: {
                  select: { slug: true, country: { select: { code: true } } },
                },
              },
            },
          },
        ...
        // الإحصائيات (filtered by country if provided)
        countryCode ? getSiteStatsByCountry(countryCode) : getSiteStats(),
      ]);
      ...
      const countries = countriesData
        ...
        .sort(
          (a, b) =>
            b.companiesCount - a.companiesCount || a.name.localeCompare(b.name)
        );
...
// دالة للحصول على إحصائيات دولة محددة
export const getSiteStatsByCountry = cache(async (countryCode: string) => {
  try {
    const [countriesCount, companiesCount, categoriesCount, reviewsCount] =
      await Promise.all([
        1, // Only one country when filtered
    ...
    console.error("خطأ في جلب إحصائيات الدولة:", error);
    return {
      countriesCount: 0,
      companiesCount: 0,
      categoriesCount: 0,
